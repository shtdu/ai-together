...
The trailing portion (after the keyword) of each step is matched to a code block, called a [step definition](https://cucumber.io/docs/cucumber/step-definitions).
...
For a less explicit alternative to `Background`, check out [conditional hooks](https://cucumber.io/docs/cucumber/api/#conditional-hooks).
...
Cucumber provides a rich API for manipulating tables from within step definitions. See the [Data Table API reference](https://github.com/cucumber/cucumber-jvm/tree/main/datatable) reference for more details.
