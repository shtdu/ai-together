...
// MyRelayToken generates a new relay token for the current user (member accessible)
...
// GetMyRelayTokenInfo returns masked token info for the current user (member accessible)
...
// RevokeMyRelayToken revokes the current user's relay token (member accessible)
