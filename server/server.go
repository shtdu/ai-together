...
func startServer() {
	...
	protected.Use(middleware.LicenseMiddleware(licenseService))
	{
		...
		// Analytics (managers only, commercial license required)
		protected.GET("/analytics/providers", middleware.RequirePermission(rbacEnforcer, "analytics", "read"), middleware.RequireCommercial(), analyticsHandlers.GetProviderAnalytics)
		protected.GET("/analytics/users", middleware.RequirePermission(rbacEnforcer, "analytics", "read"), middleware.RequireCommercial(), analyticsHandlers.GetUserAnalytics)
		protected.GET("/analytics/history", middleware.RequirePermission(rbacEnforcer, "analytics", "read"), middleware.RequireCommercial(), analyticsHandlers.GetHistory)
