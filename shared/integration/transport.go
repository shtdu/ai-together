...
// Package integration provides HTTP transport middleware for the API client.
...
type LoggingTransport struct {
	...
	logger *slog.Logger
	verbose bool // When false, only logs basic info without request/response bodies
}

// NewLoggingTransport creates a new logging transport.
// If base is nil, http.DefaultTransport is used.
// Verbose defaults to false (no request/response body logging).
...
// NewLoggingTransportVerbose creates a new logging transport with explicit verbose control.
// If base is nil, http.DefaultTransport is used.
// When verbose is true, request/response bodies are logged.
func NewLoggingTransportVerbose(base http.RoundTripper, logger *slog.Logger, verbose bool) *LoggingTransport {
	if base == nil {
		base = http.DefaultTransport
	}
	return &LoggingTransport{
...
func (t *LoggingTransport) logRequest(req *http.Request) {
	...
	// Log body separately if present and verbose mode is enabled (directly to avoid escaping)
	if t.verbose && bodyLogged && bodyStr != "" {
...
func NewTransportChain(getToken TokenGetter, logger *slog.Logger, verbose bool) http.RoundTripper {
	base := http.DefaultTransport
	logging := NewLoggingTransportVerbose(base, logger, verbose)
