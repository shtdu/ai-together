...
type APIError struct {
	...
	// RequestID is the request ID for support tracking
	RequestID string `json:"request,omitempty"`
	// ValidationErrors contains field-level validation errors (for validation errors)
...
// NewAnonymousClient creates a client without authentication (for login/register).
// Verbose flag controls whether request/response bodies are logged.
func NewAnonymousClient(baseURL string, logger *slog.Logger, verbose bool) (*ClientWithResponses, error) {
...
// Verbose flag controls whether request/response bodies are logged.
func NewAuthenticatedClient(baseURL string, getToken TokenGetter, logger *slog.Logger, verbose bool) (*ClientWithResponses, error) {
	// Create full transport chain
	transport := NewTransportChain(getToken, logger, verbose)
...
func NewAuthenticatedClientImpl(baseURL string, getToken TokenGetter, logger *slog.Logger) (ClientWithResponsesInterface, error) {
	return NewAuthenticatedClient(baseURL, getToken, logger, false)
}
