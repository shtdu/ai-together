...
// It sets the HOME environment variable to the temp directory so that
// db.Init() will create the database there instead of in the user's
// actual ~/.code-together folder.
...
func TestSeedMockRequestLogs(t *testing.T) {
	setupTestDB(t)
	// Clear existing data
	if _, err := db.DB.Exec("DELETE FROM request_log"); err != nil {
	...
	var count int
	if err := db.DB.QueryRow("SELECT COUNT(*) FROM request_log").Scan(&count); err != nil {
	...
	if err := db.DB.QueryRow("SELECT MIN(created_at), MAX(created_at) FROM request_log").Scan(&minCreated, &maxCreated); err != nil {
		t.Fatalf("range query failed: %v", err)
	}
	t.Logf("mock request_log rows=%d (%s -> %s)", count, minCreated, maxCreated)
}

// SeedMockRequestLogs generates mock request_log data, covering the last 3 months by default.
